...
jQuery(document).ready(function ($) {
	...
	$("#ui-datepicker-div").wrap('<div class="bebe_element" />');
	 ...
	 */
	$('.bebe_upload_file').change(function () {
		formfield = $(this).attr('name');
		$('#' + formfield + '_id').val("");
	});

	$('.bebe_upload_button').on('click', function () {
		var buttonLabel;
		formfield = $(this).prev('input').attr('name');
		buttonLabel = 'Use as ' + $('label[for=' + formfield + ']').text();
		tb_show('', 'media-upload.php?post_id=' + $('#post_ID').val() + '&type=file&bebe_force_send=true&bebe_send_label=' + buttonLabel + '&TB_iframe=true');
		return false;
	});

	$('.bebe_remove_file_button').on('click', function () {
    ...
    window.send_to_editor = function (html) {
		...
		if (formfield) {
			...
			if (itemurl.match(image)) {
				uploadStatus = '<div class="img_status"><img src="' + itemurl + '" alt="" /><a href="#" class="bebe_remove_file_button" rel="' + formfield + '">Remove Image</a></div>';
			} else {
				// No output preview if it's not an image
				// Standard generic output if it's not an image.
				html = '<a href="' + itemurl + '" target="_blank" rel="external">View File</a>';
				uploadStatus = '<div class="no_image"><span class="file_link">' + html + '</span>&nbsp;&nbsp;&nbsp;<a href="#" class="bebe_remove_file_button" rel="' + formfield + '">Remove</a></div>';
			}

			$('#' + formfield).val(itemurl);
			$('#' + formfield + '_id').val(itemid);
			$('#' + formfield).siblings('.bebe_upload_status').slideDown().html(uploadStatus);
			tb_remove();
